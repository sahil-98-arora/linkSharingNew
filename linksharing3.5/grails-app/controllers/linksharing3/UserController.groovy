...
//            user.save(flush:true)
            ...
            flash.messageLoginError = "Invalid Username/Password, please try again."
        }
        def postsOfTopic = Resource.list(sort:'dateCreated',order:'desc', offset:0, max:2)
        render (view: 'signup', model: [postsOfTopic:postsOfTopic])
    }
    ...
    def dashboard(){
        ...
        List<ReadingItem> subsInbox = topicService.subbedTopics(session.user)

        //topiclist for ajax
        def topicList = Topic.list()
        render (view: 'dashboard', model:[trendT:top5Trending, topicList:topicList, subsList:subsList, subsInbox:subsInbox])
    }
...
//        if(!user.active)
    ...
    def send(num, email) {
        ...
        flash.messageEmailSent = "Message containing OTP is sent to your email at "+new Date()
    ...
    def forgotPassword(){
        ...
        if(user){
            ...
            flash.messageUserNotFound = "Email ${params.email} not found"
            redirect(controller: 'user', action: 'signup')
        }
    ...
    def otpVerify(){
        ...
        if(user.otp == params.otp){
            ...
            if (params.password == params.confirmPassword){
                ...
                flash.messagePasswordNotChanged = "Password not Changed. Try again."
                redirect(action: 'signup')
            }
            //redirect(action: 'signup')
        }else{
            flash.messageWrongOtp = "OTP didn't matched"
            redirect(action: 'signup')
        }
    ...
    def sendInvitation() {
        ...
        userService.sendInvite(params)
        print("sent")
        flash.messageInviteSentSuccess = "Invitation of topic '${params.topicName}' successfully sent to '${params.email}'"
